...
export default function Models() {
  ...
  const [form, setForm] = useState({
    name: '', description: '', baseModel: 'llama-3-8b',
    quantization: 'int4', contextLength: 4096,
  });
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setShowModal(false);
      setForm({ name: '', description: '', baseModel: 'llama-3-8b', quantization: 'int4', contextLength: 4096 });
      load();
    } catch (e: any) {
      alert(e.response?.data?.message || 'Failed to create model');
    } finally {
      setSaving(false);
    }
  };
              ...
              <div key={m.id} className="card" style={{ position: 'relative' }}>
                <div style={{ padding: '18px 20px' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 14 }}>
                    {[
                      { label: 'Base Model', value: m.baseModel },
                      { label: 'Parameters', value: m.parameterCount },
                      { label: 'Quantization', value: m.quantization.toUpperCase() },
                      { label: 'Size', value: `${m.estimatedSizeGb} GB` },
                      { label: 'Context', value: `${(m.contextLength / 1024).toFixed(0)}k tokens` },
                    ].map(({ label, value }) => (
                      <div key={label}>
      ...
      {showModal && (
        <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && setShowModal(false)}>
          <div className="modal">
            ...
            <div style={{ padding: '14px 16px', background: 'var(--bg-primary)', borderRadius: 10, border: '1px solid var(--border)', marginBottom: 4 }}>
              ...
              <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                Model size varies by base + quantization. INT4 reduces memory ~4× vs FP16.
              </div>
            </div>
