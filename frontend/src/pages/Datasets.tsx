...
export default function Datasets() {
  ...
  const [form, setForm] = useState({ name: '', description: '', type: 'jsonl', tags: '' });
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setForm({ name: '', description: '', type: 'jsonl', tags: '' });
      load();
    } catch (e: any) {
        ...
        {loading ? (
          ...
          <div className="card">
            <div
              ...
              onDrop={(e) => {
                ...
                if (f) { setSelectedFile(f); setForm((p) => ({ ...p, name: f.name })); setShowModal(true); }
              }}
            >
          ...
          <div className="card">
            <div className="card-header">
              <div className="card-title"><Database size={14} /> {datasets.length} Dataset{datasets.length !== 1 ? 's' : ''}</div>
            </div>
          ...
          <div className="modal">
            ...
            <div
              ...
              onDrop={(e) => {
                e.preventDefault(); setDragOver(false);
                const f = e.dataTransfer.files[0];
                if (f) { setSelectedFile(f); if (!form.name) setForm((p) => ({ ...p, name: f.name })); }
              }}
            >
              {selectedFile ? (
                <>
                  <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--accent)' }}>{selectedFile.name}</div>
                  <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 4 }}>{formatBytes(selectedFile.size)}</div>
                </>
              ) : (
                <>
                  <Upload size={24} style={{ color: 'var(--text-muted)', margin: '0 auto 8px' }} />
                  <div style={{ fontSize: 13, color: 'var(--text-secondary)' }}>Click or drag file here</div>
                </>
              )}
            </div>
