          ...
          <div className="card">
            ...
            <div className="card-body">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className={`badge ${user.plan === 'enterprise' ? 'badge-purple' : user.plan === 'pro' ? 'badge-blue' : 'badge-gray'}`}>
          ...
          <div className="card">
            ...
            <div className="card-body">
              <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 12 }}>
                Monthly GPU compute hours included in your plan
              </div>
              <div className="flex items-center justify-between mb-2">
                ...
                <span style={{ fontFamily: 'JetBrains Mono', fontSize: 13, color: 'var(--text-primary)' }}>
                  {user.usedGpuHours}h / {user.gpuQuotaHours}h
                </span>
              </div>
              <div className="progress-bar mb-2">
                <div className="progress-fill" style={{ width: `${gpuPct}%` }} />
              </div>
              <div style={{ fontSize: 11, color: gpuPct > 80 ? 'var(--warning)' : 'var(--text-muted)' }}>
                {gpuPct}% used — {user.gpuQuotaHours - user.usedGpuHours}h remaining
              </div>
              {user.plan === 'free' && (
                <button className="btn btn-primary btn-sm mt-3">Upgrade to Pro</button>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="card">
          ...
          <div className="card-body">
            <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 16 }}>
              Use the REST API to integrate LLM Forge into your pipelines.
              Swagger docs are available at <span className="font-mono" style={{ color: 'var(--accent)', fontSize: 12 }}>http://localhost:3001/api/docs</span>
            </div>
            <div style={{ background: 'var(--bg-primary)', borderRadius: 8, padding: '12px 16px', fontFamily: 'JetBrains Mono', fontSize: 12, color: 'var(--text-secondary)', border: '1px solid var(--border)' }}>
              <span style={{ color: 'var(--text-muted)' }}>Authorization:</span> Bearer YOUR_JWT_TOKEN
            </div>
          </div>
        </div>
      </div>
