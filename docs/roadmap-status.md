...
| GGUF export and quantization     | **Partial: real LoRA/QLoRA GGUF export now runs from the backend on job completion** | `src/artifacts/artifacts.service.ts` `createLocalGgufLoraAdapter` spawns the verified `convert_lora_to_gguf.py` tool (shell-free, path-confined) when a completed local LoRA/QLoRA job requested `outputFormat: 'gguf'`; result gets exact size/SHA-256 and an authenticated local download, same as adapters; verified end to end against a real CPU LoRA training run (correct rank-16 tensor shapes) | Cover full fine-tune / merged-model GGUF and GPTQ; move from local disk to persistent object storage with expiring signed downloads                     |                       12–20 hrs |
