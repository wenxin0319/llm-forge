...
export interface CatalogModel {
  ...
  license: string;
  licenseType: 'apache-2.0' | 'mit' | 'meta' | 'llama4' | 'other';
  useCase: string;
...
export class ModelCatalogService {
  findAll(search?: string, licenseType?: string, maxParamsB?: number, minParamsB?: number, tags?: string[]): CatalogModel[] {
    return CATALOG.filter((m) => {
      ...
      if (licenseType && m.licenseType !== licenseType) return false;
      if (maxParamsB && m.paramsB > maxParamsB) return false;
      if (minParamsB && m.paramsB < minParamsB) return false;
      if (tags?.length && !tags.every((t) => m.tags.includes(t))) return false;
      return true;
    });
  }

  findOne(id: string): CatalogModel {
    const m = CATALOG.find((m) => m.id === id);
    if (!m) throw new NotFoundException(`Model '${id}' not found in catalog`);
    return m;
  }
}
