  ...
  return (
    <>
      <tr style={{ cursor: 'pointer' }} onClick={() => setExpanded((p) => !p)}>
        <td><div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>{isActive && <div className="pulse-dot" />}<strong>{job.modelName}</strong></div></td>
        <td>{job.datasetName}</td>
        <td><StatusBadge status={job.status} /></td>
        <td>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <div className="progress-bar" style={{ width: 80 }}><div className="progress-fill" style={{ width: `${job.progress}%` }} /></div>
        ...
        <td>
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }} onClick={(e) => e.stopPropagation()}>
            {isActive && <button className="btn btn-danger btn-sm" onClick={() => onCancel(job.id)}><XCircle size={12} /></button>}
        ...
        <tr>
          <td colSpan={7} style={{ padding: '0 16px 16px', background: 'var(--bg-secondary)' }}>
            <div style={{ paddingTop: 12 }}>
              ...
              <div className="log-viewer">{job.logs.map((line, i) => <div key={i}>{line}</div>)}</div>
...
export default function TrainingPage() {
  ...
  const [form, setForm] = useState({ modelId: '', datasetId: '', method: 'qlora', gpuType: 'a100-80gb', gpuCount: 1, epochs: 3, batchSize: 8, learningRate: 0.0002, useFlashAttention: true, useGradientCheckpointing: true });
  ...
  const handleLaunch = async () => {
    setLaunching(true);
    try { await api.post('/training/launch', form); setShowModal(false); load(); }
    catch (e: unknown) { const err = e as { response?: { data?: { message?: string } } }; alert(err.response?.data?.message || 'Failed to launch job'); }
    finally { setLaunching(false); }
  };

  const handleCancel = async (id: string) => {
    if (!confirm('Cancel this job?')) return;
    await api.post(`/jobs/${id}/cancel`); load();
  };
        ...
        {loading ? (
          ...
          <div className="card">
            <div className="empty-state">
              ...
              <div className="empty-state-sub">You need at least one dataset and one model</div>
              <button className="btn btn-primary mt-3" onClick={() => setShowModal(true)}><Zap size={14} /> Launch Training</button>
            </div>
          </div>
        ) : (
          <div className="card">
            <div className="card-header"><div className="card-title"><Zap size={14} /> {jobs.length} Job{jobs.length !== 1 ? 's' : ''}</div></div>
            <table className="table">
              <thead><tr><th>Model</th><th>Dataset</th><th>Status</th><th>Progress</th><th>Epoch</th><th>Cost</th><th></th></tr></thead>
              <tbody>{jobs.map((j) => <JobRow key={j.id} job={j} onCancel={handleCancel} />)}</tbody>
            </table>
          </div>
        )}
      </div>
        ...
        <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && setShowModal(false)}>
          <div className="modal">
            ...
            <div style={{ display: 'flex', gap: 16, marginBottom: 12 }}>
              {[{ key: 'useFlashAttention', label: 'Flash Attention 2.0' }, { key: 'useGradientCheckpointing', label: 'Gradient Checkpointing' }].map(({ key, label }) => (
                <label key={key} style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', fontSize: 13, color: 'var(--text-secondary)' }}>
                  <input type="checkbox" checked={(form as Record<string, unknown>)[key] as boolean} onChange={(e) => setForm((p) => ({ ...p, [key]: e.target.checked }))} />
                  {label}
                </label>
              ))}
            </div>
